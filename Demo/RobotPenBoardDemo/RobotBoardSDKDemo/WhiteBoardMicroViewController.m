...
- (void)viewWillAppear:(BOOL)animated{      // 获取笔设备  这里可以获取到 详细的笔设备相关信息
    ...
    DeviceTypes = device.deviceType == DM6?A4:device.deviceType;//用于setDeviceType传值
...
{
    CGSize sceneSize = self.WhiteBoardView.frame.size;
    CGPoint newpoint = [point getScenePointWithSceneWidth:sceneSize.width SceneHeight:sceneSize.height IsHorizontal:NO];
    point.width  = [self getPenWeight];
    point.optimizeX = newpoint.x;
    point.optimizeY = newpoint.y;
    [self.WhiteBoardView drawLineWithUtilPoint:point];
}
...
- (void)getDevicePage:(int)page andNoteId:(int)NoteId
{
    ...
    if (DeviceTypes == A4) {
        if (page == 1001) {
            NSString *path = [[NSBundle mainBundle] pathForResource:@"小学数学" ofType:@".jpg"];
            [self.WhiteBoardView insterPhotoWithPath:path];
        }else if (page == 1002)
        {
            NSString *path = [[NSBundle mainBundle] pathForResource:@"初中英语" ofType:@".jpg"];
            [self.WhiteBoardView insterPhotoWithPath:path];
        }else
        {
...
- (IBAction)ChangeImageClicked:(id)sender {
    if (![self.WhiteBoardView GetIsPhoto]) return;
    //顺时针旋转图片，每次旋转90°。
